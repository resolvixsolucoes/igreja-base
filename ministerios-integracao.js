...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  aplicarGateAbasGranular('ministerios_integracao')
  aplicarGateAcoesGranular('ministerios_integracao')
  document.querySelectorAll('[data-aba]').forEach(painel => {
    new MutationObserver(() => aplicarGateAcoesGranular('ministerios_integracao'))
...
async function resolverMinisterioId() {
  const tentativas = ['%integração%', '%integracao%', '%Integração%', '%Integracao%']
  ...
  console.error('Ministério Integração não encontrado.')
}
...
async function usuarioPodeGerenciar() {
  ...
  if (window.AUTH?.permissoesGranular) {
    for (const k in window.AUTH.permissoesGranular) {
      if (k.startsWith('ministerios_integracao::')) {
...
async function trocarAba(nome, btn) {
  ...
  if (!temPermissaoAba('ministerios_integracao', nome, 'ver')) return
        ...
        ? esc.map(e => {
            ...
            const btnEditar = (pode || ehMeu)
              ? `<button class="btn btn-secondary" style="padding:4px 10px;font-size:11px;"
                  onclick="abrirModalStatus('${e.id}','${ev.nome.replace(/'/g,"\\'")}','${ev.data}',
                  '${(e.voluntarios?.nome||'').replace(/'/g,"\\'")}',${ehMeu})">✏️</button>`
              : ''
...
function exportarCSV() {
  ...
  a.href = url; a.download = `integracao-escala-${Date.now()}.csv`; a.click()
...
function exportarPDF() {
  ...
  const doc       = new jsPDF()
  doc.setFontSize(16)
  doc.text('Relatório de Escala — Ministério de Integração', 14, 20)
  doc.setFontSize(10)
  ...
  doc.save(`integracao-escala-${Date.now()}.pdf`)
}
