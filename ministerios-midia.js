...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  aplicarGateAbasGranular('ministerios_midia')
  aplicarGateAcoesGranular('ministerios_midia')
  document.querySelectorAll('[data-aba]').forEach(painel => {
    new MutationObserver(() => aplicarGateAcoesGranular('ministerios_midia'))
...
async function resolverMinisterioId() {
  const tentativas = ['%mídia%', '%midia%', '%Mídia%', '%Midia%']
  for (const pat of tentativas) {
    const { data, error } = await _db
      .from('ministerios').select('id')
      .ilike('nome', pat).maybeSingle()
    if (!error && data) { MINISTERIO_ID = data.id; return }
  }
  console.error('Ministério Mídia não encontrado.')
}
...
async function usuarioPodeGerenciar() {
  ...
  if (window.AUTH?.permissoesGranular) {
    for (const k in window.AUTH.permissoesGranular) {
      if (k.startsWith('ministerios_midia::')) {
...
async function trocarAba(nome, btn) {
  ...
  if (!temPermissaoAba('ministerios_midia', nome, 'ver')) return
...
async function selecionarDia(dia) {
  ...
  document.getElementById('dia-painel-titulo').textContent    = `🖥️ ${dataFmt}`
...
async function renderEventos(eventos) {
  ...
  // Busca repertório de música de todos os eventos de uma vez
...
async function renderEventosDia(eventos) {
  ...
  // Busca repertório de música de todos os eventos do dia de uma vez
  const evIds = eventos.map(e => e.id)
  const { data: plData } = await _db
    .from('playlist_eventos')
    .select('evento_id, ordem, playlist_musicas(titulo, artista, tonalidade, bpm, spotify_url)')
        ...
        ? esc.map(e => {
            ...
            const btnEditar = (pode || ehMeu)
              ? `<button class="btn btn-secondary" style="padding:4px 10px;font-size:11px;"
                  onclick="abrirModalStatus('${e.id}','${ev.nome.replace(/'/g,"\\'")}',
                  '${ev.data}','${(e.voluntarios?.nome||'').replace(/'/g,"\\'")}',${ehMeu})">
                  ✏️</button>`
              : ''
  ...
  if (selecionados.length) {
    const { data: ins, error: errEsc } = await _db.from('ministerio_escala')
      .insert(selecionados.map(v => ({
        evento_id: ev.id, voluntario_id: v.id, status: 'pendente'
      }))).select()
...
async function salvarAviso() {
  ...
  const texto     = document.getElementById('inp-av-texto').value.trim() || null
  const criado_por = document.getElementById('inp-av-autor').value.trim() || null
  const arquivo   = document.getElementById('inp-av-arquivo').files[0] || null
  let arquivo_url  = null
  let arquivo_nome = null
  if (arquivo) {
    const ext  = arquivo.name.split('.').pop()
    const path = `avisos/${MINISTERIO_ID}/${Date.now()}.${ext}`
    const { error: errUp } = await _db.storage.from('ministerio-arquivos').upload(path, arquivo)
    if (errUp) { alert('Erro ao enviar arquivo.'); console.error(errUp); return }
    const { data: urlData } = _db.storage.from('ministerio-arquivos').getPublicUrl(path)
...
async function iniciarRelatorios() {
  const { data: eventos } = await _db.from('eventos_igreja')
    .select('id, nome').eq('ministerio_id', MINISTERIO_ID).order('nome')
  const sel = document.getElementById('filtro-evento')
  sel.innerHTML = '<option value="">Todos</option>'
  ;(eventos || []).forEach(ev => {
    const opt = document.createElement('option')
    opt.value = ev.id; opt.textContent = ev.nome; sel.appendChild(opt)
  })
...
async function gerarRelatorio() {
  ...
  let query = _db.from('ministerio_escala')
    ...
    .eq('eventos_igreja.ministerio_id', MINISTERIO_ID)
  if (evId)   query = query.eq('evento_id', evId)
  if (status) query = query.eq('status', status)
  const { data, error } = await query
  if (error) { console.error(error); return }
  let rows = (data || []).filter(r => r.eventos_igreja)
  if (ini) rows = rows.filter(r => r.eventos_igreja?.data >= ini)
  if (fim) rows = rows.filter(r => r.eventos_igreja?.data <= fim)
  const volIds = new Set(volsCache.map(v => v.id))
  rows = rows.filter(r => volIds.has(r.voluntario_id))
  relatorioCache = rows
  ...
  document.getElementById('stats-row').innerHTML = `
    <div class="stat-box"><div class="stat-num">${rows.length}</div>
      <div class="stat-label">Total</div></div>
    <div class="stat-box verde"><div class="stat-num">${confirmados}</div>
      <div class="stat-label">Confirmados</div></div>
    <div class="stat-box amarelo"><div class="stat-num">${pendentes}</div>
      <div class="stat-label">Pendentes</div></div>
    <div class="stat-box vermelho"><div class="stat-num">${recusados}</div>
      <div class="stat-label">Recusados</div></div>`
  ...
  rows.forEach(r => {
    ...
    const respEm  = r.respondido_em
      ? new Date(r.respondido_em).toLocaleDateString('pt-BR') : '—'
    const statusLabel = r.status === 'confirmado' ? '✅ Confirmado'
    ...
    tr.innerHTML = `
      ...
      <td><span class="pill pill-${r.status}">${statusLabel}</span></td>
      <td>${respEm}</td>`
    tbody.appendChild(tr)
  })
}
...
function exportarCSV() {
  ...
  a.href = url; a.download = 'relatorio-midia.csv'; a.click()
...
function exportarPDF() {
  ...
  const doc = new jsPDF()
  doc.setFontSize(14)
  doc.text('Relatório — Ministério de Mídia', 14, 18)
  doc.autoTable({
    startY: 26,
...
document.addEventListener('DOMContentLoaded', ativarAbaPorURL)