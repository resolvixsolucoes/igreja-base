...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Fase 7.2b — espera auth.js popular AUTH.permissoesGranular
  ...
  // aplicarGateAcoesGranular reaplica via MutationObserver porque
  // tabelas/cards sao re-renderizados ao trocar de aba ou recarregar.
  aplicarGateAbasGranular('ministerios_som')
  aplicarGateAcoesGranular('ministerios_som')
  document.querySelectorAll('[data-aba]').forEach(painel => {
    new MutationObserver(() => aplicarGateAcoesGranular('ministerios_som'))
...
async function resolverMinisterioId() {
  ...
  console.error('Ministério de Som não encontrado.')
}
...
async function usuarioPodeGerenciar() {
  ...
  if (window.AUTH?.permissoesGranular) {
    for (const k in window.AUTH.permissoesGranular) {
      if (k.startsWith('ministerios_som::')) {
...
async function trocarAba(nome, btn) {
  ...
  if (!temPermissaoAba('ministerios_som', nome, 'ver')) return
        ...
        ? esc.map(e => {
            ...
            const btnEditar = (pode || ehMeu)
              ? `<button class="btn btn-secondary" style="padding:4px 10px;font-size:11px;"
                  ...
                  ✏️</button>`
              : ''
            // Botão WhatsApp para pendentes (só admin/líder vê)
...
async function salvarAviso() {
  ...
  const { error } = await _db.from('ministerio_avisos').insert([{
    ministerio_id: MINISTERIO_ID, titulo,
...
function exportarCSV() {
  ...
  a.href = url; a.download = `som-escala-${Date.now()}.csv`; a.click()
...
function exportarPDF() {
  ...
  const doc       = new jsPDF()
  doc.setFontSize(16)
  doc.text('Relatório de Escala — Ministério de Som', 14, 20)
  doc.setFontSize(10)
  ...
  doc.save(`som-escala-${Date.now()}.pdf`)
}
